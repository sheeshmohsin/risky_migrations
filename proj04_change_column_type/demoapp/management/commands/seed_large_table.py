...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        base_datetime = timezone.make_aware(datetime(2024, 1, 1, 10, 0, 0))
        small_records = [
            SmallRecord(name=f'SmallRecord_{i}', created_at=base_datetime)
            for i in range(1, 11)
        ]
        ...
        for batch_start in range(0, large_count, batch_size):
            ...
            large_records = [
                LargeRecord(
                    ...
                    created_at=timezone.make_aware(datetime(2024, 1, ((i % 28) + 1), 10, 0, 0))
                )
